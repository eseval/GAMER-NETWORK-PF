...
export default function Chat() {
  let dataUser = !window.localStorage.userLogged
    ? ""
      ...
      <div class="flex flex-row justify-between bg-white">
        <div class="flex flex-col w-2/5 border-r-2 overflow-y-auto">
          <div class="flex flex-row overflow-x-auto items-center justify-center py-2 px-2 border-2 border-gray-200 rounded-2xl w-full">
            ...
            <button onClick={e => handleTabPendingMesseges(e)} class={`rounded-2xl w-full ${tab == 3 && nonFriends?.length === 0 ? "bg-sky-700" : tab != 3 && nonFriends?.length === 0 ? "bg-gray-100" : "bg-green-500"}`}>
              <div class="border-b-2 py-4 px-2">
                pending messeges
              </div>
            </button>
            ...
            tab === 1
              ? <div>
                ...
                {friends?.map((e, index) => {
                  ...
                  return ( //aqui mapeo los amigos y filtro los que ya tienen un chat iniciado conmigo por eso filtro los chats que son
        ...
        <div class="w-full px-5 flex flex-col justify-between">
          ...
          <div class="flex flex-col mt-5 h-max">
            ...
            {chatsLogs?.map((e, index) => {
              let messagesFiltereds = e.messages.filter(m => m[0].userId && m[0].messages != "" && m[0].messages)
              return (
                messagesFiltereds?.map((b, index) => {
                  return (
